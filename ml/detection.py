...
def get_prediction(url, model):
    ...
    if Utils.is_https(url) != True:
        ...
        output["SCORE"] = output["SCORE"] - 20  # Deduct points for no HTTPS
    ...
    domainAge = Utils.get_days_since_creation(url, 3)
    if domainAge != True:
        print("Domain is less than 3 months old")
        output["isOlderThan3Months"] = False
        output["SCORE"] = output["SCORE"] - 20  # Increased penalty for new domains
